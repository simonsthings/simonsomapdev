/** ============================================================================
 *  @file   OSAL_IsrUninstall.c
 *
 *  @path   $(DSPLINK)\gpp\src\test\osal\kernel
 *
 *  @desc   Implemetation of ISR_Uninstall API test case.
 *
 *  @ver    01.10.01
 *  ============================================================================
...
/** ============================================================================
 *  @func   OSAL_IsrUninstall
 *
 *  @desc   Tests the ISR_Uninstall API.
 *
 *  @modif  None
 *  ============================================================================
 */
EXPORT_API
DSP_STATUS
OSAL_IsrUninstall (IN Uint32 argc, IN Char8 ** argv)
{
    ...
    if ((argc != 3) || (argv == NULL)) {
        ...
        TST_PrnError ("Incorrect usage of OSAL_IsrUninstall. Status = [0x%x]",
                      status) ;
    }
    ...
    if (DSP_SUCCEEDED (status)) {
        ...
        if (DSP_SUCCEEDED (statusOfSetup)) {
            statusOfSetup = ISR_Install (NULL, isrObj) ;
        }
    }
    ...
    if (DSP_SUCCEEDED (status)) {
        if (intId < 0) {
            status = ISR_Uninstall (NULL) ;
        }
        else {
            status = ISR_Uninstall (isrObj) ;
        }
    }
