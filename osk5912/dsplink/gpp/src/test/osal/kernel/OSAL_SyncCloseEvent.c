/** ============================================================================
 *  @file   OSAL_SyncCloseEvent.c
 *
 *  @path   $(DSPLINK)\gpp\src\test\osal\kernel
 *
 *  @desc   Implemetation of SYNC_CloseEvent API test case.
 *
 *  @ver    01.10.01
 *  ============================================================================
...
/** ============================================================================
 *  @func   OSAL_SyncCloseEvent
 *
 *  @desc   Tests the SYNC_CloseEvent API.
 *
 *  @modif  None
 *  ============================================================================
 */
EXPORT_API
DSP_STATUS
OSAL_SyncCloseEvent (IN Uint32 argc, IN Char8 ** argv)
{
    ...
    if ((argc != 3) || (argv == NULL)) {
        ...
        TST_PrnError ("Incorrect usage of OSAL_SyncCloseEvent. Status = [0x%x]",
                      status) ;
    }
    ...
    /*  --------------------------------------------------------------------
     *  Tests SYNC_CloseEvent.
     *  --------------------------------------------------------------------
     */

    if (DSP_SUCCEEDED (status)) {
        if (nullTest == 1) {
            status = SYNC_CloseEvent (NULL) ;
        }
        else {
            status = SYNC_CloseEvent (evObj) ;
        }
    }
