/** ============================================================================
 *  @file   OSAL_SyncDeleteCS.c
 *
 *  @path   $(DSPLINK)\gpp\src\test\osal\kernel
 *
 *  @desc   Implemetation of SYNC_DeleteCS API test case.
 *
 *  @ver    01.10.01
 *  ============================================================================
...
OSAL_SyncDeleteCS (IN Uint32 argc, IN Char8 ** argv)
{
    ...
    if ((argc != 3) || (argv == NULL)) {
        ...
        TST_PrnError ("Incorrect usage of OSAL_SyncDeleteCS. Status = [0x%x]",
                      status) ;
    }
    ...
    if (DSP_SUCCEEDED (status)) {
        if (nullTest == 1) {
            status = SYNC_DeleteCS (NULL) ;
        }
        else {
            status = SYNC_DeleteCS (csObj) ;
        }
    }
