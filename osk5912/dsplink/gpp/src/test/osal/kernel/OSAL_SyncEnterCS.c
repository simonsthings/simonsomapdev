/** ============================================================================
 *  @file   OSAL_SyncEnterCS.c
 *
 *  @path   $(DSPLINK)\gpp\src\test\osal\kernel
 *
 *  @desc   Implemetation of SYNC_EnterCS API test case.
 *
 *  @ver    01.10.01
 *  ============================================================================
...
/** ============================================================================
 *  @func   OSAL_SyncEnterCS
 *
 *  @desc   Tests the SYNC_EnterCS API.
 *
 *  @modif  None
 *  ============================================================================
 */
EXPORT_API
DSP_STATUS
OSAL_SyncEnterCS (IN Uint32 argc, IN Char8 ** argv)
{
    ...
    if ((argc != 3) || (argv == NULL)) {
        ...
        TST_PrnError ("Incorrect usage of OSAL_SyncEnterCS. Status = [0x%x]",
                      status) ;
    }
    ...
    if (DSP_SUCCEEDED (status)) {
        if (nullTest == 1) {
            status = SYNC_EnterCS (NULL) ;
        }
        else {
            status = SYNC_EnterCS (csObj) ;
        }
    }
