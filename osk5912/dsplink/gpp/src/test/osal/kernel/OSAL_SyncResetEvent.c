/** ============================================================================
 *  @file   OSAL_SyncResetEvent.c
 *
 *  @path   $(DSPLINK)\gpp\src\test\osal\kernel
 *
 *  @desc   Implemetation of SYNC_ResetEvent API test case.
 *
 *  @ver    01.10.01
 *  ============================================================================
...
/** ============================================================================
 *  @func   OSAL_SyncResetEvent
 *
 *  @desc   Tests the SYNC_ResetEvent API.
 *
 *  @modif  None
 *  ============================================================================
 */
EXPORT_API
DSP_STATUS
OSAL_SyncResetEvent (IN Uint32 argc, IN Char8 ** argv)
{
    ...
    if ((argc != 3) || (argv == NULL)) {
        ...
        TST_PrnError ("Incorrect usage of OSAL_SyncResetEvent. Status = [0x%x]",
                      status) ;
    }
    ...
    /*  --------------------------------------------------------------------
     *  Tests SYNC_ResetEvent.
     *  --------------------------------------------------------------------
     */

    if (DSP_SUCCEEDED (status)) {
        if (nullTest == 1) {
            status = SYNC_ResetEvent (NULL) ;
        }
        else {
            status = SYNC_ResetEvent (evObj) ;
        }
    }
