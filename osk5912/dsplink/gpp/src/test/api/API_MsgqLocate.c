/** ============================================================================
 *  @file   API_MsgqLocate.c
 *
 *  @path   $(DSPLINK)\gpp\src\test\api
 *
 *  @desc   Implemetation of MSGQ_Locate API test case.
 *
 *  @ver    01.10.01
 *  ============================================================================
...
/** ============================================================================
 *  @macro  LOCATE_ATTRS_NULL/LOCATE_ATTRS_VALID
 *
 *  @desc   Type of locate attribute pointer to pass.
 *  ============================================================================
 */
#define LOCATE_ATTRS_NULL   0
#define LOCATE_ATTRS_VALID  1


/** ============================================================================
 *  @func   API_MsgqLocate
 *
 *  @desc   Tests the MSGQ_Locate API.
 *
 *  @modif  None
 *  ============================================================================
 */
EXPORT_API
DSP_STATUS
API_MsgqLocate (IN Uint32 argc, IN Char8 ** argv)
{
    DSP_STATUS      statusOfSetup       = DSP_SOK           ;
    DSP_STATUS      expStatusOfLocate   = DSP_SOK           ;
    ...
    TransportId     mqtId               = 0                 ;
    MsgQueueId      msgqId              = 0                 ;
    Uint8           msgqLocateAttrsType = LOCATE_ATTRS_NULL ;
    ...
    if ((argc != NUM_ARGS) || (argv == NULL)) {
        ...
        TST_PrnError ("Incorrect usage of API_MsgqLocate. Status = [0x%x]",
                      status) ;
    }
    ...
    if (DSP_SUCCEEDED (status)) {
        ...
        msgqLocateAttrsType = TST_StringToInt (argv [7]) ;
         ...
         *  --------------------------------------------------------------------
         */
        TST_StrToStatus (argv [1], &expStatusOfLocate) ;
        status = DSP_ENOTFOUND ;
        while ((status == DSP_ENOTFOUND) && (locateFlag == FALSE)) {
            if (msgqLocateAttrsType == LOCATE_ATTRS_NULL) {
                status = MSGQ_Locate (locateProcId, msgqId, NULL) ;
            }
            else {
                status = MSGQ_Locate (locateProcId, msgqId, &msgqLocateAttrs) ;
            }
            if (status == DSP_ENOTFOUND) {
                if (expStatusOfLocate != DSP_ENOTFOUND) {
                    TST_Sleep (1000) ;
                }
                else {
                    locateFlag = TRUE ;
         ...
         *  --------------------------------------------------------------------
         */
        statusOfCleanup = MSGQ_Release (locateProcId, msgqId) ;
