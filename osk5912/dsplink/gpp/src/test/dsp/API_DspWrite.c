 ...
 *
 *  @desc   Implemetation of DSP_Write API test case.
 *
 *  @ver    01.10.01
 *  ============================================================================
 ...
 *  @func   API_DspWrite
 *
 *  @desc   This test case tests the DSP_Write  API.
 *
 *  @modif  None.
 *  ============================================================================
 */
...
API_DspWrite (IN Uint32 argc, IN Char8 ** argv)
{
    ...
    if ((argc != 6) || (argv == NULL)) {
        ...
        TST_PrnError ("Incorrect usage of API_DspWrite. Status = [0x%x]",
                      status) ;
    }
    ...
    if (DSP_SUCCEEDED (status)) {
        do {
            if (numBytes > MAX_BUFSIZE) {
                status = DSP_Write (processorId, &dspObj, dspAddr, endianInfo,
                                    numBytes, buffer) ;
