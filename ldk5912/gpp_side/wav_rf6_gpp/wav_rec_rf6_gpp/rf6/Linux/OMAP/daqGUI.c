...
static FORM frmSample[] = { { FORM_CAPTION, 0, 0, 0, SCREEN_WIDTH, SCREEN_HEIGHT - TASKBAR_HEIGHT, &frm_caption,
		...
		{ FORM_BUTTON, PLOT, 43, 270, 37, 22, "PLOT", 0, COMPONENT_VISIBLE, FONT_SMALL }, { FORM_BUTTON, STOP, 82, 270,
				37, 22, "STOP", 0, COMPONENT_VISIBLE, FONT_SMALL },{ FORM_BUTTON, TEST_DATA_TRANSFER, 160, 270, 37, 22,"TEST", 0, COMPONENT_VISIBLE, FONT_SMALL }, { FORM_BUTTON, DRAW_ONE_GRAPH, 199, 270, 37, 22,
		...
		switch (msg.event)
		{
			...
			case EVENT_CLICK:
				switch (msg.id)
				{
					...
					case TEST_DATA_TRANSFER:
						printf("DRAW TEST_DATA_TRANSFER button Pressed\n");
						//Draw_Graph();
						plot_testblaa();
						printf("TransferTEST DONE! \n");
						break;
...
void drawGraph(struct Graph G1, GraphicsContext gc, short *aud_data1, short *aud_data2)
{
		...
		//draw_line(j + 20, 80 - ((short) G1.data[i] / y_factor), j + 1 + 20, 80 - ((short) G1.data[i + 1] / y_factor),	gc.color);
	{
	    ...
	    draw_line(j + 20, 80 - ((short) aud_data2[i] / y_factor), j + 1 + 20, 80 - ((short) aud_data2[i + 1] / y_factor),	gc.color);
    }
}
...
void plot_200x200(int num)
{
	struct Graph G1;
	GraphicsContext gc;
	int i, p;  // counters for the for-loop below.
    short data_1[300]; // some data-buffer array with 300 fields
    short data_2[300];
	// construct the Graph structure from the data:
	G1.max_y = 2500;
	G1.min_y = -2500;
